...
import "./index.css";
import { getDisplayLogoUrl } from "../../utils/check";
import { getJumpTarget } from "../../utils/setting";

const Card = ({ title, titleText, url, des, logo, resolvedLogo, catelog, onClick, index, isSearching, noImageMode, compactMode }) => {
  const [imageLoaded, setImageLoaded] = useState(false);
  const [imageError, setImageError] = useState(false);
  const [showLoading, setShowLoading] = useState(true);
  
  const imageSrc = useMemo(() => {
    return url === "admin" ? logo : getDisplayLogoUrl(logo, resolvedLogo);
  ...
  return (
    <a
      // 加密书签公开态下没有真实地址，这里必须阻止默认跳转，由外层点击逻辑负责解锁。
      href={url === "toggleJumpTarget" || !url ? undefined : url}
      onClick={() => {
        onClick();
      }}
      ...
      <div className={`card-content ${compactMode ? 'compact-mode' : ''}`}>
        ...
        <div className="card-right">
          <div className="card-right-top">
            <span className="card-right-title" title={titleText || (typeof title === "string" ? title : "")}>{title}</span>
