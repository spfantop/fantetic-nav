...
const baseUrl = "/api/";
const homeEtagStorageKey = "fantetic_nav_home_etag_v1";
// const baseUrl = "https://api.fantetic-nav.dev/api/";
...
export const FetchList = async () => {
    const etag = window.localStorage.getItem(homeEtagStorageKey) || "";
    ...
    const respEtag = response.headers?.etag;
    if (respEtag) {
        window.localStorage.setItem(homeEtagStorageKey, String(respEtag));
    }
...
export const clearHomeEtagCache = () => {
    window.localStorage.removeItem(homeEtagStorageKey);
};
