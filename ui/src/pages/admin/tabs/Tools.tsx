...
export const Tools: React.FC<ToolsProps> = (props) => {
  ...
  const handleBulkHide = useCallback(async (hidden: boolean) => {
    try {
      for (const each of selectedRows) {
        try {
          // 批量隐藏与取消隐藏复用单条更新接口，避免后端额外增加维护成本。
          await fetchUpdateTool({ ...each, hide: hidden });
        } catch (err) { }
      }
      message.success(hidden ? "批量隐藏成功!" : "批量取消隐藏成功!");
    } catch (err) {
      message.warning(hidden ? "批量隐藏失败!" : "批量取消隐藏失败!");
    } finally {
      reload();
    }
  }, [reload, selectedRows]);
  ...
  const handleBulkClearPassword = useCallback(async () => {
    try {
      for (const each of selectedRows) {
        try {
          // 批量清空密码直接复用单条更新接口，保持数据模型单一。
          await fetchUpdateTool({ ...each, accessPassword: "", clearAccessPassword: true });
        } catch (err) { }
      }
      message.success("批量清空密码成功!");
    } catch (err) {
      message.warning("批量清空密码失败!");
    } finally {
      reload();
    }
  }, [reload, selectedRows]);
  ...
  useEffect(() => {
    if (store?.tools) {
      const filteredData = store.tools
        .filter((item: any) => {
          const toolIndex = buildSearchIndex(item.name, item.desc, item.catelog);
          let show = false;
          if (searchString === "") {
            show = true;
          } else {
            show = matchesSearchIndex(toolIndex, searchString);
          }
          ...
          if (passwordFilter === "locked") {
            show = show && Boolean(item.passwordProtected);
          }
  ...
  }, [store?.tools, searchString, catelogName, passwordFilter]);
          ...
          >
            ...
            >
                  ...
                  render={(_, record: any) => {
                    return (
                      <div style={{
                        ...
                        {" "}
                          <img
                            src={record.resolvedLogo || `/api/img?url=${record.logo}`}
                            alt=""
                            width={32}
                            height={32}
                            loading="lazy"
                ...
                <Table.Column
                  ...
                  width={70}
                  render={(val) => (Boolean(val) ? "已加密" : "未加密")}
                />
                <Table.Column
                  title="网址"
                  dataIndex="url"
      ...
      >
        <Spin spinning={requestLoading}>
          <Form form={addForm}>
            ...
            <Form.Item name="accessPassword" label="访问密码" labelCol={{ span: 4 }}>
              <Input.Password placeholder="可选，填写后前台访问该书签需输入密码" />
            </Form.Item>
            <Form.Item
      ...
      >
        <Spin spinning={requestLoading}>
          <Form form={updateForm}>
            ...
            <Form.Item name="accessPassword" label="新密码" labelCol={{ span: 4 }}>
              <Input.Password placeholder="留空表示保持现有密码不变" />
            </Form.Item>
            <Form.Item name="clearAccessPassword" label="清空密码" labelCol={{ span: 4 }}>
              <Select
                options={[
                  { label: "否", value: false },
                  { label: "是", value: true },
                ]}
                placeholder="是否清空当前密码"
              />
            </Form.Item>
